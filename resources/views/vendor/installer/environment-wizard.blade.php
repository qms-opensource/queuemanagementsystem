...
@section('container')
    <div class="tabs tabs-full">
        ...
        <form method="post" id="wizardForm3" autocomplete="off" action="{{ route('LaravelInstaller::environmentSaveWizard') }}" class="tabs-wrap">
            <div class="tab" id="tab1content">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">

                <div class="form-group {{ $errors->has('app_name') ? ' has-error ' : '' }}">
                    <label for="app_name">
                        {{ trans('installer_messages.environment.wizard.form.app_name_label') }}<span class="req">*</span>
                    </label>
                ...
                <div class="form-group {{ $errors->has('app_url') ? ' has-error ' : '' }}">
                    <label for="app_url">
                        {{ trans('installer_messages.environment.wizard.form.app_url_label') }}<span class="req">*</span>
                    </label>
                    <input type="text" name="app_url" id="app_url" value="" placeholder="{{ trans('installer_messages.environment.wizard.form.app_url_placeholder') }}" />
                ...
                <div class="buttons">
                    <button class="button next-step" id="showDatabaseSettings"  >
            ...
            <div class="tab" id="tab2content">
                ...
                <div class="form-group {{ $errors->has('database_name') ? ' has-error ' : '' }}">
                    <label for="database_name">
                        {{ trans('installer_messages.environment.wizard.form.db_name_label') }}<span class="req">*</span>
                    </label>
                ...
                <div class="form-group {{ $errors->has('database_username') ? ' has-error ' : '' }}">
                    <label for="database_username">
                        {{ trans('installer_messages.environment.wizard.form.db_username_label') }}<span class="req">*</span>
                    </label>
                ...
                <div class="form-group {{ $errors->has('database_password') ? ' has-error ' : '' }}">
                    ...
                    <input type="password" name="database_password" id="database_password" value="" autocomplete="new-password" placeholder="{{ trans('installer_messages.environment.wizard.form.db_password_placeholder') }}" />
                ...
                <div class="buttons">
                    <button class="button" id="showApplicationSettings" >
            ...
            <div class="tab" id="tab3content">
                <div class="block">
                    ...
                    <div class="info">
						...
						<div class="form-group {{ $errors->has('name') ? ' has-error ' : '' }}">
                            <label for="user_name">{{ trans('installer_messages.environment.wizard.form.app_tabs.admin_user_name_label') }}<span class="req" style="display: inline; margin: 0px; padding: 0px;">*</span></label>
                            <input type="text" name="name" id="user_name" value="" placeholder="{{ trans('installer_messages.environment.wizard.form.app_tabs.user_name_placeholder') }}" />
                            @if ($errors->has('name'))
                                <span class="error-block">
                                    <i class="fa fa-fw fa-exclamation-triangle" aria-hidden="true"></i>
                                    {{ $errors->first('name') }}
						...
						<div class="form-group {{ $errors->has('email') ? ' has-error ' : '' }}">
                            <label for="user_email">{{ trans('installer_messages.environment.wizard.form.app_tabs.admin_user_email_label') }}<span class="req" style="display: inline; margin: 0px; padding: 0px;">*</span></label>
                            <input type="text" name="email" id="email" value="" placeholder="{{ trans('installer_messages.environment.wizard.form.app_tabs.user_email_placeholder') }}" />
                            @if ($errors->has('email'))
                                <span class="error-block">
                                    <i class="fa fa-fw fa-exclamation-triangle" aria-hidden="true"></i>
                                    {{ $errors->first('email') }}
                        ...
                        <div class="form-group {{ $errors->has('password') ? ' has-error ' : '' }}">
                            <label for="password">{{ trans('installer_messages.environment.wizard.form.app_tabs.admin_user_password_label') }}<span class="req" style="display: inline; margin: 0px; padding: 0px;">*</span></label>
                            <input type="password" name="password" id="user_password" value="" placeholder="{{ trans('installer_messages.environment.wizard.form.app_tabs.user_password_placeholder') }}" />
						...
						<div class="form-group {{ $errors->has('app_display_name') ? ' has-error ' : '' }}">
                            <label for="app_display_name">{{ trans('installer_messages.environment.wizard.form.app_tabs.app_display_name_label') }}<span class="req" style="display: inline; margin: 0px; padding: 0px;">*</span></label>
                            <input type="text" name="app_display_name" id="app_display_name" value="" placeholder="{{ trans('installer_messages.environment.wizard.form.app_tabs.app_display_name_placeholder') }}" />
                            @if ($errors->has('app_display_name'))
                                <span class="error-block">
                                    <i class="fa fa-fw fa-exclamation-triangle" aria-hidden="true"></i>
                                    {{ $errors->first('app_display_name') }}
                                </span>
                            @endif
                        </div>
						<!-- strat here -->
				...
				<div class="buttons">
                    <button class="button" id="applicationSettings" class="button" type="submit">
                        {{ trans('installer_messages.environment.wizard.form.buttons.install') }}
                        <i class="fa fa-angle-right fa-fw" aria-hidden="true"></i>
                    </button>
                </div>
...
<script type="text/javascript" src="//cdn.jsdelivr.net/jquery.bootstrapvalidator/0.4.5/js/bootstrapValidator.min.js"></script>
<script type="text/javascript" src='http://cdnjs.cloudflare.com/ajax/libs/bootstrap-validator/0.4.5/js/bootstrapvalidator.min.js'></script>
	<script>
		  $(document).ready(function () {	
			$('[id=showDatabaseSettings]').on("click", function (e) {  
				ValidateFormData();		
				var validator = $('[id*=wizardForm]').data('bootstrapValidator');
				validator.validate();
			...
			$('[id=showApplicationSettings]').on("click", function (e) { 		
				ValidateFormData();		
				var validator = $('[id*=wizardForm]').data('bootstrapValidator');
				validator.validate();
				if(validator.isValid()){
			...
			$('[id=applicationSettings]').on("click", function (e) { 		
				ValidateFormData();		
				var validator = $('[id*=wizardForm]').data('bootstrapValidator');
				validator.validate();
				if(validator.isValid()){
					// document.getElementById('tab3').checked = true;
					$('#applicationSettings').attr('disabled', false);
					return validator.isValid();
				} 
			  var validator='';
			});
			$(".tab-label").on('click',function(){
				...
				validator.validate();
				if(validator.isValid()){
					if($(this).prev("input.tab-input").length)
					{
						$(this).prev("input.tab-input").attr("disabled",false);
						$('#applicationSettings').attr('disabled', false);
					}
					return validator.isValid();
				} else {
					if($(this).prev("input.tab-input").length)
					{
						if($(this).prev("input.tab-input").attr('id') != 'tab1')
						{
							$(this).prev("input.tab-input").attr("disabled",true);
						}
					}
			...
			$(".tab-input").on("change", function (e) {	
				    var childId = $(this).attr('id');				  
					ValidateFormData();		
					var validator = $('[id*=wizardForm]').data('bootstrapValidator');
					//validator.validate();
		...
		function ValidateFormData() {
			$('[id*=wizardForm]').bootstrapValidator({
				framework: 'bootstrap',
				inclusion : [':hidden', ':not(:visible)'],
				fields: {
					app_name: {
						validators: {
							notEmpty: {
					...
					environment: {
						validators: {
							notEmpty: {
								message: 'Please select enviornment'
							}
						}
					},
					app_url: {
					...
					app_display_name: {
						validators: {
							notEmpty: {
								message: 'Please enter Application Display Name'
							}
						}
					},
					app_display_system: {
						validators: {
							notEmpty: {
								message: 'Please enter Application Display System'
							}
						}
					},
